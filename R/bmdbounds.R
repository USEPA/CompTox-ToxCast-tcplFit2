...
bmdbounds = function(fit_method, bmr, pars, conc, resp, onesidedp = .05, bmd = NULL, which.bound = "lower", poly2.biphasic = TRUE,top_dir){
  ...
  if(which.bound == "lower") {
    ...
    ys = sapply(X = xs, FUN = bmdobj, fname = fname, bmr = bmr, conc = conc, resp = resp, ps = pars, mll = maxloglik,
                onesp = onesidedp, partype = 2,poly2.biphasic = poly2.biphasic,top_dir = top_dir)
  ...
  out = try(uniroot(bmdobj, bmdrange, fname = fname, bmr = bmr, conc = conc, resp = resp, ps = pars, mll = maxloglik,
                onesp = onesidedp, partype = 2,poly2.biphasic = poly2.biphasic,top_dir = top_dir)$root)
