...
This vignette contains details of all available models in `tcplfit2` package with parameter explanations and illustrative plots. It is to note that all models are implemented with the assumption that the baseline response is always 0. 
...
poly1_plot <- ggplot(mapping=aes(ex_conc)) +  
  ...
  scale_color_manual(name='a values',
                     ...
                     values=c('a=-5'='black', 'a=0.05' = 'red', 'a=10'='blue', 'a=55'='slateblue3'))

poly1_plot
```

**Figure 1:** This plot illustrates how changes in slope `a` affect the lines.

## Polynomial 2

Polynomial 2 is a quadratic model fit. Note, the version implemented in `tcplfit2` is re-parameterized to assume the baseline response is always zero, and the `a` and `b` parameters are interpreted in terms of their impact on the the x- and y-scale, respectively.
...
**Figure 2:** Plot on the left illustrates how changes in parameter `a` affect the shapes of polynomial 2 curves, while holding `b` constant. Plot on the right illustrates how changes in parameter `b` affect the shapes of polynomial 2 curves, while holding `a` constant.
...
**Figure 3:** Plots of power model use a smaller concentration range of 0 to 3 to better show the impact of `p` on resulting curves. Plot on the left illustrates how changes in parameter `a` affect the shapes of power curves, while holding `p` constant. Plot on the right illustrates how changes in parameter `p` affect the shapes of power curves, while holding `a` constant.
...
* `p` : Power, also called the hill coefficient. Mathematically, it is a measure of how steep the response curve is. In context, it is a measure of the cooperativity of substrate binding to the enzyme, transporter, etc. `p` is restricted such that $0.3 \le p \le 8$. 
...
**Figure 4:** It is to note that the x-axes are in log-10 scale because this model is optimized in log-10 scale with log hill model $f(x) = \frac{tp}{1 + 10^{(p*(ga-x))}}$. Plot on the top left corner illustrates how changes in parameter `tp` affect the shapes of hill curves, while holding the other parameters constant. Plot on the top right corner illustrates how changes in parameter `ga` affect the shapes of hill curves, while holding the other parameters constant. Plot on the bottom left corner illustrates how changes in parameter `p` affect the shapes of hill curves, while holding the other parameters constant.

## Gain-Loss
Model: $f(x) = \frac{tp}{[(1 + (ga/x)^p )(1 + (x/la)^q )]}$

Gain-Loss is the product of two hill models with one simulates the response going up and one simulates the responses going down. A gain-loss curve can gain first and then lose, or the opposite way. 

The definitions of `tp`, `ga`, and `p` are the same as in [hill model](#hill), and `la` is the counterpart of `ga`, `q` is the counterpart of `p` in the loss half of the curve. 
...
**Figure 5:** It is to note that the x-axes are in log-10 scale because this model is optimized in log-10 scale with log gain-loss model $f(x) = \frac{tp}{[(1 + 10^{(p*(ga-x))} )(1 + 10^{(q*(x-la))} }$. Plot on the left illustrates how changes in parameter `la` affect the shapes of gain-loss curves, while holding the other parameters constant. Plot on the right illustrates how changes in parameter `q` affect the shapes of gain-loss curves, while holding the other parameters constant.
...
**Figure 6:** Plots of exponential 2 model use a smaller concentration range of 0 to 3 to better show the impact of `b` on resulting curves. Plot on the left illustrates how changes in parameter `a` affect the shapes of exponential 2 curves, while holding `b` constant. Plot on the right illustrates how changes in parameter `b` affect the shapes of exponential 2 curves, while holding `a` constant.
...
**Figure 7:** This plot uses a smaller concentration range of 0 to 3 to better show the impact of `p` on resulting curves. This plot illustrates how changes in parameter `p` affect the shapes of exponential 3 curves, while holding the other parameters constant.
...
* `ga` : AC50, concentration at 50% of maximal activity. It acts as slope and controls how fast the curve is approaching the top. If `ga` increases, the curve is shrunk horizontally. `ga` is restricted to be $0 \le ga < \infty$ (e.g. non-negative). 
...
**Figure 8:** Plot on the left illustrates how changes in parameter `tp` affect the shapes of exponential 4 curves, while holding `ga` constant. Plot on the right illustrates how changes in parameter `ga` affect the shapes of exponential 4 curves, while holding `tp` constant.
...
**Figure 9:** This plot illustrates how changes in parameter `p` affect the shapes of exponential 5 curves, while holding the other parameters constant.
...
*This table provides a summary of `tcplfit2` model details. This table is taken from the [Data Processing vignette](https://cran.r-project.org/web/packages/tcpl/vignettes/Data_processing.html#concentration-response-modeling-details) from `tcpl` package.*
