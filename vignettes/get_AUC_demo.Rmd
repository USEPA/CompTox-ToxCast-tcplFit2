...
This vignette will walk you through how to generate area under the curve (AUC) for Concentration-Response curves with `tcplfit2`. AUC can be think of a measure of overall efficacy/potency and users may want to have it as part of the analysis. The following sections will show examples of how to obtain AUC in various applications and the rationale behind the function.


# Main Feature 

Now one can obtain the area under the curve (AUC) value for the winning model alongside with hit-calling results. Use `auc = TRUE` in `concRespCore` function to include AUC in the output. This argument defaults to `TRUE`, one might exclude it from the output by setting it to `FALSE`. 
...
The following sections demonstrate how one can obtain AUC when they do curve fitting and hit-calling separately with `tcplfit2_core` and `tcplhit2_core` instead of in one-setting, and how AUC function behave with different types of curves. 

# Applications
## Positive Curves
This section provides an example of how to use `get_AUC` function in `tcplfit2` to calculate area under the curves (AUC) for dose-response curves fitted. The following chunk of code fits all available models in `tcplfit2` with some example data. `verbose = TRUE` will return the output with Akaike information criterions (AICs) of each model and the final winning model, and `do.plot = TRUE` returns a figure with all the curves.
...
output <- tcplfit2_core(conc, resp, .8, do.plot = TRUE)
```
...
for (model in fitmodels){
  ...
  # get AUC
  mylist[[fit_method]] <- get_AUC(fit_method, min(conc), max(conc), modpars)
...
In the example above all the curves are positive increasing curves. This session will examine how this function behave with negative curves. Taking example 3 from the vignette which gives data for negative curves and curves that have area both above and below 0.
...
Please note that the x-axis in plot above is in regular unit so it looks different from the figure above which uses log-conc for x-axis. 

For decreasing curves, the function will return a negative value when integrating over a negative curve. However, some users may want to consider all "areas" positive. In this case they can use `return.abs = TRUE` argument to convert negative AUC to positive when return. 

## Bi-phasic Curves

For now, none of the models in this package would be able to fit a bi-phasic curve. This session shows what the function will do if the user input a bi-phasic curve.  

The following chunk simulates a polynomial 2 curve that cross x-axis. Please note that the parameterization of poly2 in `tcplfit2` is a little different from normal polynomial 2.
...
If we integrate over a curve that has areas below and above 0, the function will return area above the curve minus area blow the curve (blue-yellow). In this example, area above the curve for the simulated curve is slightly larger than the area below the curve so it returns a positive value.  

AUC is used as an indicator of overall effect size, so for curves that pass the x-axis, it makes more sense to report the total area of the sections above and below x-axis, that is, blue area plus yellow area. The following code shows how to get the total area in this example. One would need to identify where the curve crosses x-axis first.  
...
In some scenarios, users might only run the `concRespCore` function and get information about the winning model and the hitcall. In this case, the output from `concRespCore` and `tcplhit2_core` would be a one row data frame that contains concentration data, winning model name and the parameter values. 

There is a wrapper function `post_hit_AUC` available for calculating AUC after hit-calling that will extract information from this type of output so one don't need to manually input the model info into the format `get_AUC` takes, as demonstrated above. 

The winning model from data used in example 1 is Hill. Compare the AUC from above and the AUC returned from the `post_hit_AUC` function, they are the same.

```{r example 4}
out <- tcplhit2_core(output, conc, resp, cutoff, onesd = 0.4)
out
post_hit_AUC(out)
```
