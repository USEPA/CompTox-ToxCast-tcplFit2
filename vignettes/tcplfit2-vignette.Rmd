...
After all models are fit to the data, `tcplhit2_core` is used to perform hitcalling, which  corresponds to Level 5 in `tcpl`. The continuous hitcall value (`hitc`) is the product of three proportional weights, and the resulting continuous value is between 0 and 1. The proportional weights as defined as:
...
$$ p_1 = 1 - \frac{exp(0.5*AIC_{const})}{exp(0.5*AIC_{const})+exp(0.5*AIC_{winning})}$$
...
$p_2$
: “at least one median response is greater than the cutoff.”
...
$p3$
: “the top of the fitted curve is above the cutoff”
...
where $MLL$ is the maximum log-likelihood of the original predicted best fit model, $LL$ is the log-likelihood of the re-scaled predicted best fit model, and the $\pm$ is $+$ when $ |top| \geq | cutoff | $ and $-$ when $ |top| < | cutoff | $.

See Sheffield et al., 2021 for more information on tcplfit2.

The following plots provide visual representations for the comparisons conducted in each of the proportional weights that make up the continuous hitcall value. Each figure has one item "highlighted" in blue and another "highlighted" in red.  The blue represents the reference for the proportional weight of interest, whereas the red represents an indicator for potential a biologically active response (i.e. key comparator) for the proportional weight of interest.  For example, for $p_1$ which (as mentioned previously) is meant determine whether the winning model (red), which is the best fit curve to the observed data given it has the lowest AIC, is much different from the constant model (blue), which indicates no biological response.
...
```{r hitc_plots,echo=FALSE}
## Generate a Base Plot for the Concentration-Response ##
base_plot <- ggplot2::ggplot()+
  geom_point(aes(log10(conc),resp))+
  geom_line(aes(x = log10(XC),y = YC))+
  geom_hline(aes(yintercept = c(-1,1)*DATA_CASE[,"cutoff"],linetype = "Cutoff"))+
  geom_hline(aes(yintercept = c(-1,1)*1.349*DATA_CASE[,"onesd"],linetype = "BMR"))+
  ggplot2::ylim(c(-1,1))+
...
p1_plot <- base_plot+
  ...
  # add the constant model (reference)
  ggplot2::geom_hline(aes(yintercept = 0,color = "Constant"))+
  # add the winning model (comparison)
  ggplot2::geom_line(aes(x = log10(XC),y = YC,color = "Best Fit"))+
  scale_color_manual(name = "",
                     breaks = c("Constant","Best Fit"),
                     values = c("blue","red"))+
  theme(legend.position = "bottom")

## Proportional Weight 2 ##
# obtain the concentrations that are outside the cutoff band
p2_DG <- which(abs(resp)>=abs(DATA_CASE[,"cutoff"]))
# generate the plot
p2_plot <- base_plot+
  # add a title for the subplot
  ggplot2::ggtitle("p2",subtitle = "Responses Outside Cutoff")+
  # add the cutoff band - highlighted
  geom_hline(aes(yintercept = c(-1,1)*DATA_CASE[,"cutoff"],linetype = "Cutoff",color = "Cutoff"))+
  # add the concentrations with median responses outside the cutoff band - highlighted
  ggplot2::geom_point(aes(log10(conc)[p2_DG],resp[p2_DG],color = "Extreme Responses"))+
  scale_color_manual(name = "",
                     breaks = c("Cutoff","Extreme Responses"),
                     values = c("blue","red"))+
  theme(legend.position = "bottom")

## Proportional Weight 3 ##
...
p3_plot <- base_plot+
  ...
  # add the cutoff band - highlighted
  geom_hline(mapping = aes(yintercept = DATA_CASE[,"cutoff"],color = "Top at Cutoff"),linetype = "dashed")+
  # add the original predicted curve - highlighted
  geom_line(aes(x = log10(XC),y = YC,color = "Best Fit"))+
  # add the rescaled predicted curve - highlighted
  geom_line(aes(x = log10(XC),y = YC_rescale,color = "Rescaled Best Fit"))+
  # add the 'top' (maximal predicted change in response from baseline)
  geom_hline(mapping = aes(yintercept = fit$exp4$top,color = "Top"))+
  # scale_linetype_manual(name = "",
  #                       breaks = c("Cutoff","BMR","Top"),
  #                       values = c("dashed","dotted","solid"))+
  scale_color_manual(name = "",
                     breaks = c("Top at Cutoff","Best Fit","Rescaled Best Fit","Top"),
                     values = c("hotpink","blue","red","skyblue"))+
  theme(legend.position = "bottom")

  

## All Plots ##
grid.arrange(p1_plot,p2_plot,p3_plot,ncol = 3,
             top = paste(DATA_CASE[,"signature"],DATA_CASE[,"dtxsid"],sep = "\n"),
             left = "response",
             bottom = paste("log10(conc)",
                            paste(paste("hitc:",signif(hit[,"hitcall"],3)),paste("bmd:",signif(hit[,"bmd"],3)),sep = ", "),
                            sep = "\n")
             )
```
...
Users may consider binarizarizing the continuous hitcall values into active or inactive designations, setting the the activity threshold based on the level of stringency required by the user. Currently the ToxCast work, requires a `hitc` value greater than or equal to 0.90 to label a response as active, and anything less is considered inactive. For further details on the activity threshold used in ToxCast we refer readers to the `tcpl` vignette and Nyffeler et al., 2023.
