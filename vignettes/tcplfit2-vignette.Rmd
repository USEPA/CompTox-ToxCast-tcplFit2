...
`concRespCore` is the main wrapper function that utilizes two other utility functions, `tcplfit2_core` and `tcplhit2_core`, to perform curve-fitting, hit-calling and potency estimate calculations. This example will show the use of `concRespCore` and next section will show the use of `tcplfit2_core` and `tcplhit2_core` separately. The first argument to the `concRespCore` function is a named list called 'row' which should contains the following key inputs:
...
The `row` object can have other optional elements which will be included in the function output. These can be, for instance, the name of the chemical (or other identifiers) or the name of the assay being modeled. These are couple other parameters might be used:
...
If the `fitmodels` parameter is specified, then it must always include the constant model (`cnst`) since this provides one basis for comparison during hitcalling. However, some models may be excluded, for example the gain-loss (`gnls`) model is excluded in some applications.
...
The output can be passed into `concRespPlot2` to get a plot of the winning curve. This function returns a basic `ggplot`, and the intention is to take the advantage of the flexibility and allow users to add their own customized layers of detail. For more refer to the plotting vignette.

```{r example1 plot, fig.height = 4.55, fig.width = 8}
# plot the winning curve from example 1, add a title
concRespPlot2(res, log_conc = TRUE) + ggtitle("Example 1 some chemical")
```

**Figure 1:** This figure shows the winning curve for a single concentration-responses calculation. The concentrations (x-axis) are in $log_{10}$ units.

## Example 2: Running a series of concentration-response models for a single assay

This example shows fitting a series of concentration-response models for a single assay using `tcplifit2_core` and `tcplhit2_core` sequentially. Using them sequentially allows the users to exam the output from `tcplifit2_core` which contain all model fits for a response. The output from `tcplifit2_core` can be passed into `plot_allcurves` to plot all curves fitted for a chemical.
...
```{r example2, warning=FALSE}
  ...
  for(spid in spid.list) {
    ...
    # Input concentrations and response, cutoff, and a list of models
    ...
    # Pass the output from tcplfit2_core into tcplhit2_core along with cutoff, onesd,
    # and any identifiers
    ...
    # store all result in one table
    result_table <- rbind(result_table,out)
  }

```

Show all the plots generated in the loop:

```{r example2 plot1, fig.height = 8, fig.width = 7}
grid.arrange(grobs=plt_lst,ncol=2)
```

**Figure 2:** These figures are example plots generated using the `plot_allcurves` function. Each plot depicts all the concentration-response model fits to an example chemical. In the plots, each model is represented with a different color and the concentrations (x-axis) are in `log10` units.

One would typically save the result rows in a data frame and export these for further analysis, as shown in the loop. One could also use `concRespPlot2` to plot selected rows/results (e.g. those with significant responses) like how it was demonstrated in Example 1.
...
The first step is to establish the concentration index. This is consider level-1 in `tcpl`. The concentration indices are integer values ranking N distinct concentrations from 1, for being the lowest, to N, the highest. This index can be used to calculate the baseline median absolute deviation for an assay.
...
Next is to do data adjustment. This is consider level-2 in `tcpl`. Generally if the response values (`rval`) need to be logged or transformed in some way from their original values this is where that adjustment would occur. Transformed response values are referred to as corrected values and are stored in the `cval` field/variable. However, in this case, the corrected values (`cval`) are identical to the original response values (`rval`). Meanwhile, data with poor well quality (`wllq = 0`) and missing response values are removed.
...
The data need to be normalized and zero-centered before model fitting. The example data has neutral control and negative control available, and this example will show how to normalize responses to a control. However, this process varies from assay to assay and the steps shown in this example are applicable to only the example data set. In other applications such as when neutral control or positive/negative controls are not available, the user shall normalize the responses in a way that is reasonable to their data. Provided below is a list of normalizing methods used in `tcpl` for reference.

That being said, for this example, the normalized response (`resp`) is calculated as a percent of control, i.e. the ratio of the difference between the corrected (`cval`) and baseline (`bval`) values divided the difference between the positive/negative control (`pval`) and baseline (`bval`).
...
Before model fitting, we need to determine the median absolute deviation (`BMAD`) around baseline and one standard deviation (`onesd`) of the noise around the baseline, which will later be used for cutoff and benchmark response (`BMR`) calculations. This is consider part of level-4 in `tcpl`. `tcplfit2` assumes no effect is happening in the two lowest concentrations. With this assumption, we calculate `BMAD` and `onesd` as the median absolute deviation and standard deviation of the data from the two lowest concentrations of the test wells.
...
After the data adjustment and normalization, the model fitting can begin. In `tcpl`, this would be considered level 4, and is where `tcplfit2` is used to fit all of the models as a dependency for `tcpl`.
...
After all of the models have been fit, `tcplhit2_core` can be called sequentially for hitcalling. The output of level 4 can be fed directly into the `tcplhit2_core` function. The results are then pivoted and shown in the resulting datatable.
...
The output table resulting from the previous code chunk is the same format as the `result_table` table in example 2. Thus, one can use the `concRespPlot2` function, as done previously in example 2, to plot the results. The next code chunk demonstrates how to visualize the example 3 fit results.
...
**Figure 4:** Each plot displays the winning curve for one sample in the `subdat` dataset.

## Example 4: How to "Censor" the BMD
...
concRespPlot2(res_low, log_conc = T) + 
  geom_rect(aes(xmin = res_low[1, "bmdl"],xmax = res_low[1, "bmdu"],ymin = 0,ymax = 30),
            alpha = 0.05,fill = "skyblue") + 
  geom_segment(aes(x = res_low[, "bmd"], xend = res_low[, "bmd"], y = 0, 
                   yend = 30),col = "blue")
```
...
**Figure 8**: This plot shows the estimated BMD and the confidence interval before and after "censoring". The dashed dark violet line represents the threshold dose and the solid brown line represents the minimum tested concentration, which are at 0.24 and 0.6, respectively. The estimated BMD of 0.302 falls between the threshold and reference doses, which leaves the BMD and confidence intervals unchanged. Here, the estimated BMD and "censored" BMD are the same. Thus, the green and blue lines and "X"s representing the estimated BMD before and after "censoring", respectively, as well as their confidence intervals indicated by the shaded regions completely overlap.

# Appendix {#appendix}
Example 1 and 2 illustrated two plotting functions available in `tcplifit2` that are `ggplot` based. This appendix will show two other based R plotting functionalities available in `tcplfit2`: the `do.plot` argument in `concRespCore` and `concRespPlot`.

The `concRespPlot` function and the `do.plot` argument in `concRespCore` provide plots similar to figure 1 and 2, respectively. The `do.plot` argument returns a plot of all curve fits of a chemical, and `concRespCore` returns a plot of the winning curve and the hitcall results.
