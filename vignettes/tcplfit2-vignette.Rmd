...
The above table is identical to the mc0 table that would be seen in invitrodb and recognized by `tcpl`.  However, some users may want to run standalone `tcplfit2` fitting without the need for a MySQL database connection to invitrodb.  The first steps (levels 1 through 3 in `tcpl`) are used to adjust the data and determine the baseline sampling variability, or noise, that will later be used for cutoff calculation. For further information on normalization and determination of a cutoff, see vignettes for library(tcpl) at https://CRAN.R-project.org/package=tcpl .
...
## acid = assay component identifier; srcf = source file name; apid = assay plate identifier; coli = column index (location on assay plate); rowi = row index (location on assay plate); spid = sample identifier; conc = concentration
...
There are much fewer level 4 methods, but generally it is a requirement to assign a method that calculates the bmad and assign a method that calculates the standard deviation of the noise.
...
After all methods up to level 4 have been applied, the model fitting steps can begin.  In tcpl this would be considered level 4, and is where `tcplfit2` is used to fit all of the models as a dependency of tcpl.



```{r example4_fitting, fig.height = 6, fig.width = 7, eval = F}
...
```{r example4_hitcalling, fig.height = 6, fig.width = 7, eval = F}
