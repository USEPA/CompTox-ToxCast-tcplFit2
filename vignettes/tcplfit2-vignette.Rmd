...
The estimated BMD can occurs above or below an experimental dose or other specified values; e.g., it is higher or lower than the maximum or minimum concentration tested in the data. When this happens, `tcplhit2_core` and `concRespCore` provide options for users to "censor" BMD with use of arguments `bmd_low_bnd` and `bmd_up_bnd`. This section will provide examples to demonstrate how to do that. 

`bmd_low_bnd` and `bmd_up_bnd` are multipliers applied to the minimum or maximum concentrations in the data to generate lower or upper bound for BMD estimates. 

### "Censor" BMD with a lower bound {#censorlowerbound}
First, let's consider a situation when BMD is below the lowest concentration tested. This situation could happen when the experiment was not designed to have enough low dose groups to capture behavior in low dose region and it can be a red flag for unreliable analysis. The following code chunk loads data of a sample from `mc3` and simulates an example like this. Data from the first couple low dose groups were removed so that the responses in the first couple groups left would increase quickly. 
```{r example 5 lower}
...
The lowest concentration in the data is 0.6 but the estimated BMD from the function is 0.302, it is lower than the minimum concentration tested. One can allow BMD to be lower than the minimum concentration tested while restrict it to be no lower than a percentage of that by using the argument `bmd_low_bnd`. For example, if one sets this argument to 0.8, the BMD is not allowed to be lower than 80% of the lowest concentration in the data (i.e., 0.8*0.6 = 0.48). If the BMD is lower than the minimum concentration but not lower than this lower bound, the function will returned it as-is. However, in this example, 0.302 is still lower than 0.48, so the function will force BMD to be on the lower bound and shift the original BMD boundaries by the difference between the BMD estimate and the lower bound. One might set `bmd_low_bnd` to 1 to avoid BMD to be lower than the minimum concentrate in the data. 
...
**Figure 7**: This plot shows how BMD and its boundaries changed after "censoring". The green line and green "X" mark the BMD before "censoring" and the green column shows the original boundaries. The blue line and blue "X" mark the "censored" BMD and the blue column shows the updated boundaries. The violet dash line represents the minimum concentration in the data, and the brown dash line represents the threshold value set by `bmd_low_bnd`. In this example, BMD was "censored" to the threshold value, and its boundaries were shifted right.
...
If the estimated BMD falls between the minimum or maximum concentration and the threshold bounds set by `bmd_low_bnd` or `bmd_up_bnd`, the result will be returned as-is. 
