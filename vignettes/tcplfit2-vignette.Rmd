...
DT::datatable(res_low2[1, c("Name",'Min. Conc.',"bmd", "bmdl", "bmdu")],rownames = FALSE)
```

```{r example 5 lower cont., echo=FALSE}
...
**Figure x**: This plot shows how BMD and its boundaries changed after "censoring". The green line and green "X" mark the BMD before "censoring" and the green column shows the original boundaries. The blue line and blue "X" mark the "censored" BMD and the blue column shows the updated boundaries. The BMD and its boundaries were forced to be shifted right because it's not allowed to be lowered than 0.48. Note that the updated boundaries also became narrower.  
...
The BMD 299.927 is higher than the maximum concentration in the data, which is 80. One can set `bmd_up_bnd` to 1 to use the maximum concentration as the upper bound (i.e., BMD is not allowed to be larger than 100% of the maximum concentration in the data). And if BMD is bigger than this upper bound, it will be forced to be the maximum concentration. One can also input a fraction to `bmd_up_bnd` so BMD is not allowed to be larger than a percentage of the maximum concentration. For example, set it to 0.6 to have BMD no larger than 60% percentage of the maximum concentration. Doing this can avoid the maximum concentration to be the BMD and make sure BMD is in the dose range tested so the analysis can have some dose groups below the BMD and some dose groups above BMD.
```{r example upper 2, eval=FALSE, echo=FALSE}
...
```{r example 5 upper cont., echo=FALSE, eval=FALSE}
...
res_up3 <- concRespCore(row,fitmodels = c("cnst", "hill", "gnls", "poly1", "poly2", 
                                          "pow", "exp2", "exp3", "exp4", "exp5"), 
                        conthits = T, aicc = F, bidirectional=F, bmd_up_bnd = 0.6)
```

```{r example upper-2, echo=FALSE}
res_up3['Max Conc.'] <- max(conc)
res_up3['Name'] <- name
res_up3[1, c("Max Conc.", "bmd", "bmdl", "bmdu")] <- round(res_up3[1, c("Max Conc.", "bmd", "bmdl", "bmdu")], 3)
