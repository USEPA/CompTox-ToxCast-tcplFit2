...
**Figure 1:** This plot depicts the fitted concentration-response curves, with each model represented in a different color. Note, concentrations (x-axis) are in log-10 units. 
...
**Figure 2:** Each figure shows the curve-fit results for a random sample in the `mc3` data. The title contains the chemical name and assay ID. Each plot displays the summary of curve-fit results -- including the winning model, AC50, top, BMD, ACC, and hitcall -- at the top of the plot. The black dots indicate observed responses, and the winning model fit is displayed as the solid black curve. The estimated BMD is displayed by the solid green vertical line, and the solid green lines bounding the shaded green region indicates the estimated confidence bounds on the BMD (i.e., lower and upper BMD confidence bounds - BMDL and BMDU, respectively). The black horizontal lines bounding the shaded grey region centers at x-axis indicates the estimated noise region.
...
**Figure 3:** This figure provides an example of plots generated using the `tcplfit2` plotting option through the `do.plot=TRUE` argument in the `concRespCore` function. Each plot displays all model fits for one row of data in the `signatures` dataset.
...
**Figure 4:** This figure provides an example of plots generated using the `concRespPlot` function. Each plot displays the winning curve with a summary of the fitting for one row of data in the `signatures` dataset.
...
**Figure 5:** Each figure plots the winning curve and displays some of the hit-call results for one sample in the `subdat` dataset.
...
**Figure 6**: This plot shows the estimated BMD and confidence interval before and after "censoring". The green line and the "X" mark the estimated BMD before "censoring" along with the green shaded region representing the estimated confidence interval. The blue line and the "X" mark the estimated BMD after "censoring" along with the blue shaded region representing the estimated confidence interval. The solid brown line represents the minimum concentration in the data, and the dashed dark violet line represents the threshold dose set by `bmd_low_bnd`. In this example plot, the estimated BMD and the boundaries were shifted right such that the BMD was "censored" to the threshold value (blue "X" and region).
...
**Figure 7**: This plot shows the estimated BMD and confidence interval before and after "censoring". The green line and the "X" mark the BMD before "censoring" along with the green shaded region representing the estimated confidence interval. The blue line and the "X" mark the "censored" BMD along with the blue shaded region representing the estimated confidence interval. The solid brown line represents the maximum concentration in the data, and the dashed dark violet line represents the threshold dose set by `bmd_up_bnd`. In this example plot, the estimated BMD and the boundaries were shifted left such that the BMD was "censored" to the threshold value (blue "X" and region).
...
**Figure 8**: This plot shows the estimated BMD and the confidence interval before and after "censoring" with a threshold dose of 0.24. The dashed dark violet line represents the threshold dose, and the solid brown line represents the minimum tested concentration. The estimated BMD 0.302 falls between the reference dose and the threshold dose and therefore remains unchanged. In this example plot, the estimated BMD and "censored" BMD are the same. Thus, the green and blue lines and the "X"s that mark the estimated BMD before and after "censoring" , respectively, along with their shaded regions indicating the estimated confidence bounds are completely overlapped. 
