...
```{r example1,class.source="fold-show",warning=FALSE}
...
Once the fitting funcion is set up, one can perform dose-response modeling for all `spid`'s in the dataset. **Warning: The fitting step on the full data set, `dat`, can take 7-10 minutes with a single core laptop.** Thus, the following hidden code chunk demonstrates how to curve fit the full example dataset, but is not executed.
...
## Ploting All Models with `concRespCore` and `concRespPlot`
...
plot(conc_ex2, pred_resp,xlim = c(0,1))
...
mll
: Maximum log-likelihood of winning model. Used in calculating the continuous hit call  $length(modpars) - aic(fit_{method})/2$
