...
This example shows fitting a series of concentration-response series in a single assay using the `tcplifit2_core` and `tcplhit2_core` functions sequentially. Using the functions sequentially allows users greater flexibility to exam the intermediate output. For example, the output from `tcplifit2_core` contains model parameters for all model fit to concentration-response series provided. Furthermore, `tcplifit2_core` results may be passed to `plot_allcurves` to generate a comparative plot of all curves fit to a concentration-response series.
