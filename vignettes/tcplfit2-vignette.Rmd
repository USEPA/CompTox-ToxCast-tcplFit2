...
**Figure 1:** This plot depicts the fitted concentration-response curves, with each model represented in a different color. Note that the x-axis uses log-10 concentrations. 
...
**Figure 2:** Each figure shows the curve-fit result of a random sample from `mc3` data. Each plot has a title that contains the chemical name and assay ID. On the top of each plot displays a summary of the fitting, including what is the winning model, AC50, top, BMD, ACC, and hit-call. The concentration-response observations are the black dots, and the winning model curve is depicted by the black curve. If BMD is not `NA`, it and its boundaries are highlighted by a green line and a green rectangle. The gray bar around the x-axis represents the estimated noise region. 
...
**Figure 3:** These figures are plots generated from using one of the plotting options in `tcplfit2`: using the `do.plot=TRUE` argument in `concRespCore` function. Each figure plots all models used to fit one row of data in `signatures`.
...
**Figure 5:** Each figure plots the winning curve and displays some of the hit-call result of one sample in `subdat`.
