...
# Glossary
The following glossary, though it may not be encompassing all terms included this package, is provided to serve as a quick reference when using `tcplfit2`:

a
: Model fitting parameter in the following models: exp2, exp3, poly1, poly2, pow

ac5
: Active concentration at 5% of maximal modeled response (top) value

ac10
: Active concentration at 10% of maximal modeled response (top) value

ac20
: Active concentration at 20% of maximal modeled response (top) value

ac50
: Active concentration at 50% of maximal modeled response (top) value

acc
: Active concentration at cutoff

ac1sd
: Active concentration at 1 standard deviation of baseline response
...
bmr
: Benchmark Response. Level at which BMD is calculated as $onesd*bmr_scale$, where the default bmr_scale is 1.349

caikwt
: Akaike weight of constant model relative to winning model, calculated as $exp(-aic(cnst)/2)/(exp(-aic(cnst)/2) + exp(-aic(fit method)/2))$. Used in calculating the continuous hitcall.

conc
: Tested concentrations, typically uM
...
n_gt_cutoff
: Nnumber of data points above the cutoff
...
rmse
: Root mean square deviation of the data points relative to model fit. Lower RMSE indicate model fits the data well.

top_over_cutoff
: Ratio of the maximal modeled response value to the cutoff (top/cutoff)

top
: Response value at highest concentration or modeled top value (tp) 
