...
This example shows fitting a series of concentration-response series in a single assay using the `tcplifit2_core` and `tcplhit2_core` functions sequentially. Using the functions sequentially allows users greater flexibility to examine the intermediate output. For example, the output from `tcplifit2_core` contains model parameters for all model fit to concentration-response series provided. Furthermore, `tcplifit2_core` results may be passed to `plot_allcurves` to generate a comparative plot of all curves fit to a concentration-response series.
...
The following code demonstrates how to set up the input data and execute curve-fitting and hitcalling with the `tcplfit2_core` and `tcplhit2_core` functions.
...
The output from `tcplifit2_core` is a list that contains: nested lists named by the models, `modelnames` which is vector of the fitted model names, and `errfun` which tracks what distribution was assumed for errors. Each nested list contains details of the fitted model in its name. 
```{r example 2 fit results}
# shows the structure of the output object from tcplifit2_core
summary(model_fits[[1]])
```
Look into what is inside the "hill" list for example: 
`success`, an indicator which 1 indicates the fit was successful; `COV`, an indicator which 1 indicates the inverted hessian was successfully found; fitted parameter values and parameter standard deviations; `modl` which are the model fitted values; AIC, root mean square error (RME), top and AC50. 
All of these details are provided for other models too except for the constant model. For constant model, only the `success`, AIC, RME and fitted `er` value will be provided. 
```{r}
summary(model_fits[[1]][["hill"]])
```
...
**Figure 2:** Example plots generated from `plot_allcurves`. Each plot depicts all model fits for a given sample. In the plots, each model is represented with a different color and line type, and the concentrations (x-axis) are in $log_{10}$ units.

One would typically save the result rows in a data frame and export these for further analysis, as shown in the loop above. One can also use `concRespPlot2` to plot selected rows/results (e.g. those with significant responses) similar to what is shown in [Example 1](#ex1).
...
**Figure 3:** Concentration-response data and the winning curve for Bisphenol A using the `concRespPlot2` function. The concentrations (x-axis) are in $log_{10}$ units.

## Example 3: Running tcpl-like multi-concentration response data without a database connection {#ex3}
