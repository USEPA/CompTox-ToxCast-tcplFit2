...
The function `concRespCore` can also have other optional elements which will be included in the output. These can be, for instance, the name of the chemical (or other identifiers) or the name of the assay being modeled. These are couple other parameters might be used: 

* `conthits` - Boolean argument. If TRUE (the default, and recommended usage), the hitcall
returned will be a continuous value between 0 and 1. 
* `errfun` - Allows user to specify distribution of errors. The default is "dt4", models are fitted assuming the errors follow a Student's t-distribution with a degree of freedom of 4. Can assume normal distributed errors by changing it to "dnorm".
* `poly2.biphasic` - If TRUE (the default, and recommended usage), the polynomial 2 model will fit a biphasic curve to the response (i.e. increase then decrease or vice versa). Can force monotonic fitting with FALSE (parabola with the vertex centers at 0).
* `do.plot` - If this is set to TRUE (default is FALSE), a plot of the curve will be generated. The user can also select only a subset of the models to be run. The example below has all of the possible ones included. 
...
**Figure 1:** This plot depicts all the concentration-response model fits to an example dataset.  Each model is represented with a different colors and concentrations (x-axis) are in $log_{10}$ units. 
...
Example 1 and 2 illustrated two base R plotting functionalities using `do.plot` argument in `conRespCore` and `concRespPlot`. This example illustrates two more `ggplot2` based plotting functionalities available in `tcplfit2`, namely `plot_allcurves` and `concRespPlot2`. These two plotting functions provides plots similar to figure 1 and 2, respectively. However, `ggplot` allows flexibility of adding customized layers of detail. This example will focus on showing how to use the functions, for more details refer to the plotting vignette. 
...
```{r example3, fig.height = 6, fig.width = 7, warning = FALSE}
  ...
  # set up empty list to store plots and fitted results for later
  res <- NULL
  plt_lst <- NULL
  # fit 6 observations in signatures
  for(i in 1:nrow(signatures)){
    # set up input data
    conc = as.numeric(str_split(signatures[i,"conc"],"\\|")[[1]])
    ...
    model_fits <- tcplfit2_core(conc, resp, cutoff = cutoff)
    plt_lst[[i]] <- plot_allcurves(model_fits, conc, resp, log_conc = TRUE)
    out <- tcplhit2_core(model_fits, conc, resp, cutoff = cutoff, onesd = onesd,
                         identifiers = list(name = name, assay = assay))
    if(i==1){
      res <- out
    }else{
...
do.call(grid.arrange, plt_lst)
```

**Figure 3:** This figure provides several example plots generated using the `plot_allcurves` function. Each plot displays data for a single row of data in the `signatures` dataset, and like Figure 1 provides all model fits for a given response.

```{r example3_plot2, fig.height = 8, fig.width = 7}
plt_lst_2 <- NULL
  # plot results using `concRespPlot2`
  for(i in 1:nrow(res)){
    plt_lst_2[[i]]<- concRespPlot2(res[i,], log_conc = TRUE)
  }
do.call(grid.arrange, plt_lst_2)
```

**Figure 4:** This figure provides example plots generated using the `concRespPlot2` function. Each plot displays the winning curve for a single row of the `signatures` dataset.
...
### Normalization
The data need to be normalized and zero-centered before model fitting.

Since neutral control and negative control are available in the example data, this example will show to normalize responses to a control. Calculate the normalized response (`resp`) as a percent of control, i.e. the ratio of the difference between the corrected (`cval`) and baseline (`bval`) values divided the difference between the positive/negative control (`pval`) and baseline (`bval`).

The baseline values are calculated as the medians of all data in neutral control wells by plate and, in this example, the negative control is calculated as the plate-wise minimum, by assay plate ID (`apid`), of the medians of corrected value (`cval`) of negative control wells.

If positive/negative control is not available, `pval` is assumed 0. In cases where neutral control is not available, the baseline value (`bval`) can be calculated as the median of the test well responses from the two lowest concentration (concentration index is 1 or 2), and calculate the normalized response as the ratio of the difference between the corrected (`cval`) and baseline (`bval`) values divided the baseline (`bval`).
...
Before model fitting, we need to determine the median absolute deviation (BMAD) around baseline and one standard deviation (onesd) of the noise around the baseline, which will later be used for cutoff and benchmark response (BMR) calculations. This is consider part of level-4 in `tcpl`. Here we calculate BMAD and onesd as the median absolute deviation and standard deviation of the data from the two lowest concentrations of the test wells.

BMD can be calculated as the median absolute deviation of the data in control wells too. Check out
other methods of determining BMAD and onesd used in `tcpl`.
...
BMAD and onesd should be calculated on the assay level. The example data set only contains one assay compound, so only need to calcuate once.
...
```{r example4_plot, fig.height = 8, fig.width = 7}
  ...
  for(i in 1:nrow(res)){
    concRespPlot(res_wide[i,])
  }
```

**Figure 5:** Each plot displays the winning curve and hit-call results for one sample in the `subdat` dataset.
