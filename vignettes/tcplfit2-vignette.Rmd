...
min(conc2)
# function results
DT::datatable(res_low[1, c("bmd", "bmdl", "bmdu")],rownames = FALSE,options = list(scrollX = T))
```
...
DT::datatable(res_low2[1, c("bmd", "bmdl", "bmdu")],rownames = FALSE,options = list(scrollX = T))
```

**upper bound** discrepancies between the documentation and actual behavior? 
...
max(conc)
# function results
DT::datatable(res_up[1, c("bmd", "bmdl", "bmdu")],rownames = FALSE,options = list(scrollX = T))
```

The BMD 300 is higher than the maximum concentration tested, 80. 

The documentation use 10 as example, but I think there are two seniros from here: 
