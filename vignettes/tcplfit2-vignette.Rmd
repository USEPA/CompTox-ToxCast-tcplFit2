...
For example, we want the estimated BMD to be no lower than 80% of the reference dose. We can use `bmd_low_bnd = 0.8` to set a threshold dose of 0.48. If the estimated BMD is lower than the threshold dose (like in this example), it will be "censored" to the threshold dose, and its confident boundaries will be shifted right by a distance equal to the difference between the estimated BMD and the threshold dose. Figure 7 provides a visual representation of this process. Valid inputs for `bmd_low_bnd` are numbers between 0 and 1, not including 0.
...
If the estimated BMD falls between the reference dose and the threshold dose, the estimated BMD will remain unchanged. For demonstration purposes, we use the lower bound censoring example, but the same principle applies to the upper bound censoring case.
...
**Figure 8**: This plot shows how BMD and its boundaries changed after "censoring" with a threshold dose of 0.24. The dark violet dashed line represents the threshold dose, and the brown solid line represents the minimum concentration in the data. BMD = 0.302 falls between the reference dose and the threshold dose, so it will remain unchanged. A green column and a green "X" should mark the original BMD estimate and its boundaries, and a blue column and a blue "X" should mark the "censored" BMD and its boundaries. In this case, they are completely overlapped. 
