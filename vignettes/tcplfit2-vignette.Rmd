...
**Figure 3:** These plots demonstrate the plots generated using the `tcplfit2` plotting option through the `do.plot=TRUE` argument in the `concRespCore` function. Each figure displays all model fits to one row of data in the `signatures` dataset.
...
**Figure 4:** These figures are plots generated from using one of the plotting option in `tcplfit2`: using the `concRespPlot` function. `concRespPlot`, when takes output from `concRespCore`, plots the winning curve with additional hit-call information. 
...
**Figure 7**: This plot shows the estimated BMD and confidence interval before and after "censoring". The green line and the "X" mark the BMD before "censoring" along with the green shaed region representing the estimated confidence interval. The blue line and the "X" mark the "censored" BMD along with the blue shaded region representing the estimated confidence interval. The solid brown line represents the maximum concentration in the data, and the dashed dark violet line represents the threshold dose set by `bmd_up_bnd`. In this example plot, the estimated BMD and the boundaries were shifted left such that the BMD was "censored" to the threshold value (blue "X" and region).
