...
* `ga` : AC50, concentration at 50% of the maximal activity. It provides useful information about the "apparent affinity" of the protein under study (enzyme, transporter, etc.) for the substrate. The parameter constraint on `ga` is $0 \le ga < \infty$. 
...
**Figure 25:** The top left plot illustrates how changes in `tp` (maximal change in response) affect the shape of resulting Hill curves, while holding all other parameters constant ($ga = 5, p = 1.76$). The top right plot illustrates how changes in `ga` (slope) affect the shape of resulting Hill curves, while holding all other parameters constant ($tp = 120, p = 1.76$). The bottom left plot illustrates how changes in `p` (power) affect the shape of resulting Hill curves, while holding all other parameters constant ($tp = 120, ga = 5$). Note: The x-axes are in $log_{10}$ scale to reflect the scale the model is optimized in, i.e. log Hill model $f(x) = \frac{tp}{1 + 10^{(p*(ga-x))}}$.
...
* `la` : Loss AC50, concentration at 50% of the maximal activity in the loss direction. The larger it gets the slower the curve starts to decrease (if it increase first). The parameter constraints on `la` are $0 \le la < \infty$ and $la-ga\ge 1.5$.
* `q` : Loss power or the rate of loss. The larger it is, the faster the curve decreases (if it increases first). `q` is restricted such that $0.3 \le q \le 8$. 
...
* `b` : The x-scalar. If `b` increases, the curve is shrunk horizontally. `b` is restricted such that $b > 0$ (i.e. positive). 
...
* `p` : Power. A measure of how steep the curve is. The further `p` is from 1, the steeper the curve is. `p` is restricted such that $0.3 \le p \le 8$. 
...
* `ga` : AC50, concentration at 50% of the maximal activity. It acts as the slope, controlling the rate at which the response (curve) approaches the top. If `ga` increases, the curve is shrunk horizontally. `ga` is restricted such that $0 \le ga < \infty$ (i.e. non-negative). 
