...
```{r example4_cndx, fig.height = 6, fig.width = 7, warning=FALSE}
library(tcpl)
...
Since neutral control and negative control are available in the example data, this example will show how to normalize responses to a control. The normalized response (`resp`) is calculated as a percent of control, i.e. the ratio of the difference between the corrected (`cval`) and baseline (`bval`) values divided the difference between the positive/negative control (`pval`) and baseline (`bval`).

The baseline (`bval`) values are calculated as the medians of all data in neutral control wells by plate and, in this example, the negative control (`pval`) value is calculated as the plate-wise minimum, by assay plate ID (`apid`), of the medians of corrected value (`cval`) of negative control wells.

`tcplfit2` assumes no effect is happening in the two lowest concentrations. With this assumption, in cases where neutral control is not available, the baseline value (`bval`) can be calculated as the median of the test well responses where concentration index is 1 or 2. The normalized responses can be calculated as the ratio of the difference between the corrected (`cval`) and baseline (`bval`) values divided the baseline (`bval`).
