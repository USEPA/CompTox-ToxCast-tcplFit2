...
First, let's consider a situation when the estimated BMD is below the lowest tested concentration. When this happened, it implies that the experiment was not designed to have enough low-dose groups to capture behaviors in the low-dose regions and it can be a red flag for unreliable analysis. 

In the following code chunk, we use some data from `mc3` to simulate an example of this case. We take the data of a sample and remove data from dose groups that are below 0.41. The lack of data in low-dose regions can cause the estimated BMD to be lower than the lowest concentration left in the data. 
```{r example 5 lower}
...
The lowest concentration (i.e. reference dose) in the data is `r min(conc2)` but the estimated BMD from the hit-calling result is `r round(res_low$bmd, 3)`, which is lower. Users can allow the estimated BMD to be lower than the reference dose while restricting it to be no lower than a threshold dose set by using the argument `bmd_low_bnd`. 
...
Let's consider another situation when the estimated BMD is higher than the maximum concentration in the data. In the following chunk, we take some example data from `mc3` and run a concentration-repose fitting and hit-calling. This is an example in which the estimated BMD is a lot larger than the maximum concentration in the data. We will show how to use `bmd_up_bnd` to top extreme BMD estimates by an upper bound. 
```{r example 5 upper}
# load example data
...
The BMD `r round(res_up$bmd, 3)` is higher than the maximum concentration (i.e. reference dose) in the data, which is `r max(conc)`. Similarly with `bmd_low_bnd`, we can allow BMD to be larger than the reference dose but not larger than a threshold dose set by using `bmd_up_bnd`. For example, we don't want the estimated BMD to be larger than 2 times the reference dose. We can use `bmd_up_bnd = 2` to set a threshold dose of 160. If the estimated BMD is bigger than this threshold dose (like in this example), it will be "censored" to the threshold dose, and its boundaries will be shifted left. Figure 9 provides a visual representation of this process. Valid inputs for `bmd_up_bnd` are numbers larger than or equal to 1.
...
**Figure 9**: This plot shows how BMD and its boundaries changed before and after "censoring". The green line and green "X" mark the BMD before "censoring" and the green column represents its boundaries. The blue line and blue "X" mark the "censored" BMD and the blue column represents its boundaries. The brown solid line represents the maximum concentration in the data, and the dark violet dashed line represents the threshold value set by `bmd_up_bnd`. In this example, BMD was "censored" to the threshold value, and its boundaries were shifted left.

### "Censor" BMD with tcplhit2_core
`bmd_low_bnd` and `bmd_low_bnd` are presented in `tcplhit2_core`. `concRespCore` "inherit" them through leveraging `tcplhit2_core` within the function. That means, if one chooses to make calls to `tcplfit2_core` and `tcplhit2_core` separately, they can do the same BMD "censoring" with `tcplhit2_core`. The results from using these two arguments in `concRespCore` or in `tcplhit2_core` are identical. Codes are provided below to show how to replicate the same result from the ["censor" with lower bound example](#censorlowerbound) above with `tcplhit2_core`.
...
If the estimated BMD falls between the the reference dose and the threshold dose, the estimated BMD will remain unchanged. For demonstration purposes, we use the lower bound censoring example, but the same principle applies for the upper bound censoring case.

We use the same data from the ["censor" with lower bound example](#censorlowerbound) and show what happens if we use a lower threshold dose. Let's assume it is acceptable as long as the estimated BMD is not lower than 40% (two fifths) of the minimum concentration (i.e. reference dose) in the data. The estimated BMD `r res_low$bmd` is between the reference dose `r min(conc2)` and the new threshold dose 0.24. The BMD estimates and its boundaries will remain unchanged. 
...
**Figure 8**: This plot shows how BMD and its boundaries changed after "censoring" with a threshold dose of 0.24. The dark violet dashed line represents the threshold dose, and the brown solid line represents the minimum concentration in the data. BMD = 0.302 falls between the reference dose and the threshold dose, so it will remain unchanged. A green column and a green "X" should mark the original BMD estimate and it's boundaries, and a blue column and a blue "X" should mark the "censored" BMD and its boundaries. In this case, they are completely overlapped. 
